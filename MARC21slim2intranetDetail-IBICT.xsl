...
<xsl:stylesheet version="1.0"
 ...
 <xsl:variable name="SubjectModifier"><xsl:if test="marc:sysprefs/marc:syspref[@name='TraceCompleteSubfields']='1'">,complete-subcampo</xsl:if></xsl:variable>
 ...
 <xsl:choose>
 <xsl:when test="$leader19='a'">Alterar</xsl:when>
 <xsl:when test="$leader6='a'">
 <xsl:choose>
 <xsl:when test="$leader7='c' or $leader7='d' or $leader7='m'">Texto</xsl:when>
 <xsl:when test="$leader7='i' or $leader7='s'">
 <xsl:choose>
 <xsl:when test="substring($controlField008,22,1)!='m'">Recursos continuado</xsl:when>
 <xsl:otherwise>Séries</xsl:otherwise>
 </xsl:choose>
 </xsl:when>
 ...
 <xsl:when test="$leader6='k'">Fotografia</xsl:when>
 <xsl:when test="$leader6='r'">Objeto </xsl:when>
 ...
 <span class="results_summary suppressed_opac">Oculto no OPAC</span>
 </xsl:if>
 </xsl:if>
 ...
 <span class="results_summary analytics"><span class="label">Analíticos: </span>
 <a>
 <xsl:choose>
 ...
 <xsl:text>Exibir analiticos</xsl:text>
 </a>
 </span>
 </xsl:if>
 ...
 <xsl:text>Exibir volumes</xsl:text>
 </a>
 </span>
 </xsl:if>
 ...
 <span class="results_summary set"><span class="label">Configurar: </span>
 ...
 <span class="label">Tipo da mídia: </span>
 ...
 <xsl:choose>
 <xsl:when test="@ind2=0"> [Pedaço do título]</xsl:when>
 ...
 <xsl:when test="@ind2=4"> [Título da capa]</xsl:when>
 <xsl:when test="@ind2=5"> [Título de página título adicionado]</xsl:when>
 <xsl:when test="@ind2=6"> [Título da legenda]</xsl:when>
 <xsl:when test="@ind2=7"> [Título de execução]</xsl:when>
 <xsl:when test="@ind2=8"> [Título da lombada]</xsl:when>
 </xsl:choose>
 </xsl:if>
 ...
 <span class="results_summary contained_works"><span class="label">Obras contidas: </span>
 ...
 <span class="results_summary genre"><span class="label">Gênero/Forma: </span>
...
<!-- MARC21 776 Additional Physical Form Entry -->
 ...
 <span class="label">Formatos físicos adicionais:</span>
...
<!-- DDC classification -->
 ...
 <span class="results_summary ddc">
 <span class="label">Classificação Decimal de Dewey: </span>
...
<!-- LOC classification -->
 ...
 <span class="label">Classificação da LoC: </span>
...
<!-- Other classification -->
 ...
 <xsl:with-param name="codes">qui</xsl:with-param>
 </xsl:call-template>
 </xsl:when>
 <xsl:otherwise>
 <xsl:call-template name="subfieldSelectSpan">
 ...
 <span class="label">Nota de ação: </span>
 ...
 <span class="results_summary holdings_note basic_bibliographic_unit"><span class="label">Reservas: </span>
 ...
 <span class="results_summary opac_view"><span class="label">Visualização OPAC: </span>
 <a><xsl:attribute name="href"><xsl:value-of select="$OPACBaseURL"/>/cgi-bin/koha/opac-detail.pl?biblionumber=<xsl:value-of select="str:encode-uri(marc:datafield[@tag=999]/marc:subfield[@code='c'], true())"/></xsl:attribute><xsl:attribute name="target">_blank</xsl:attribute>Abrir em uma nova janela</a>.
 </span>
 </xsl:if>
